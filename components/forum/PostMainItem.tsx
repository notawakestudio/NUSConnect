...
const PostMainItem = ({ id }): JSX.Element => {
  ...
  return (
    <div className="p-12 flex flex-col items-start overflow-hidden shadow-lg rounded-lg h-full w-full ">
      <h2 className="sm:text-3xl text-2xl title-font font-medium text-gray-900 mt-4 mb-4">{currentPost.title}</h2>
      ...
      <div className="flex items-center flex-wrap pb-4 mb-4 border-b-2 border-gray-100  w-full">
        <div className="flex flex-wrap justify-start items-center mt-4">
          {tags.map((tag) => (
            <div key={currentPost.id} className="text-xs mr-2 py-1.5 px-4 text-gray-600 bg-blue-100 rounded-2xl">
              #{tag}
            </div>
          ))}
        </div>
        <span className="text-gray-400 mr-3 mt-4 inline-flex items-center ml-auto leading-none text-sm pr-3 py-1 border-r-2 border-gray-200">
          <svg className="w-4 h-4 mx-2" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
